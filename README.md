...
<img src="assets/streaming-before-after.svg" alt="修复前后对比：40 次串行 PUT 拖 10 秒 vs 3 次合并推送 1.5 秒" width="920">
...
> 每次卡片内容 PUT 是一次 ~300ms 的网络往返，且所有更新走**串行队列**。一次 2.6 秒生成的回复，排队了 **40 个 PUT**，硬生生拖成 **10 秒**的"打字机"。
...
对照 `audit_events`（`agent.run.started` / `agent.run.finished`）：模型流式生成 **2.6s** 就结束了，但 PUT 队列拖了 **10s**。约 40 次 PUT × ~300ms/次，完全吻合——卡顿不在生成侧，在推送侧。
